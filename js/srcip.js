...
Player.prototype.roll = function() {
	 //*****Random Number******//
  this.dieRoll1= this.die1[Math.floor(Math.random()*this.die1.length)];
  this.dieRoll2= this.die2[Math.floor(Math.random()*this.die1.length)];
  console.log(die1);
  console.log(die2);
  if(this.dieRoll1 === 1)&&(this.dieRoll2 === 1) {
    this.currently = 0;
    counter++;
  }
  else {
    this.currently += this.dieRoll1+this.diceRoll2;
  }
}
...
$(function() {
  ...
  function compRoll(player, otherPlayer, turnPlayer) {
    (setTimeout(function() {
      player.roll();
      $(".dieValue").text(player.dieRoll1);
	  $(".dieValue").text(player.dieRoll2);
      $("#player1Currently").text(player.currently);
	  
	  $("#player11Currently").text(player.currently);
      if(player.total + player.currently >= 20) {
        winner(turnPlayer);
      }
      if(player.currently < 15 && (player.dieRoll1 !== 1 && player.dieRoll2 !== 1)) {
        compRoll(player, otherPlayer, turnPlayer);
      }
      else if (player.dieRoll1 === 1)&&(player.dieRoll2 === 1){
       $("#player1Currently").text(player.currently);
	    $("#player11Currently").text(player.currently);
  ...
  $("form").submit(function(event) {
    ...
    if (player0.name === "" || player1.name === ""){
      ...
      $("#rollButton").click(function(){
        ...
        if(counter % 2 === 0) {
          player0.roll();
          $(".dieValue").text(player0.dieRoll1);
		  $(".dieValue").text(player0.dieRoll2);
          $("#player0Currently").text(player0.currently);
		  $("#player00Currently").text(player0.currently);
          if (player0.dieRoll1 === 1)&&(player0.dieRoll2 === 1) {
      ...
      $("#holdButton").click(function() {
		...
		$("#player00Total").text(player0.total);
        playerTurn("player1", "player0");
        $("button").prop("disabled", true);
        compRoll(player1, "player0", "player1");
        if(player0.total >= 20) {
